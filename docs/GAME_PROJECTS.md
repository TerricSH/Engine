...
GPU frame safety: destruction defers the backend resource removal to the start of the next rendered frame (a frames-in-flight boundary), where the Vulkan backend's `remove_resource` performs its own `wait_idle` before freeing buffers — a mesh referenced by an in-flight frame is never freed mid-frame. Re-creating a mesh under the same name before that drain cancels the pending removal because the next upload replaces the old buffers through the backend's existing wait-idle-guarded replacement path.

Diagnostics: `runtime_mesh_memory()` (also surfaced as `RuntimeDiagnostics.runtime_meshes`) reports live mesh/vertex/index counts, vertex and index payload bytes, and the number of GPU removals awaiting the next frame boundary. Uploads ride the standard per-frame sync (backends dedupe unchanged content by hash); there is no separate runtime-mesh upload scheduler — per-frame streaming cost stays governed by the cooked-asset stream budget above. C# script exposure of mesh creation is intentionally not part of this API; it arrives with the game-side binding work, and terrain (ENG-T0) is the first consumer.
