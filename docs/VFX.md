...
`ParticleEmitter` is a deterministic CPU emitter. It supports continuous
emission and a one-shot burst, a hard particle budget, random lifetime and
speed ranges, a directional cone, acceleration, angular velocity, and linear
start-to-end size interpolation. Live particles are world-space and are
rebased during a world-origin shift.
For a non-looping emitter, `duration` limits emission; zero means unlimited.

Particles render as camera-facing quads through the ordinary
`RenderFrameInput` path. They therefore reuse material alpha modes, backend
asset synchronization, frustum/layer culling, and transparent surface sorting.
...
a smoke, spark, or dust texture without changing the emitter.
...
This is the production-facing foundation, not a full effects editor. It does
not yet provide GPU particle simulation, per-particle color/alpha curves,
collision or sub-emitters, soft-particle depth fading, or volume-projected
decals with receiver masks and normal blending. Decals are surface-aligned
meshes, so a single decal does not wrap around corners.
