...
fn chain_positions(global: &[BoneTransform], bones: &[BoneIndex]) -> Vec<Vec3> {
    bones.iter().map(|b| global[b.0 as usize].translation).collect()
}
...
fn solve_fabrik(
    ...
    for _pass in 0..5 {
        for i in (0..n - 1).rev() {
            ...
            let child_pos = if effector.weight >= 1.0 - 1e-6 {
                ...
                current_global[child_bone.0 as usize].translation
    ...
    let d = final_global[chain.bones[0].0 as usize].translation.distance(target);
...
fn solve_ccd(
    ...
    for _ in 0..chain.iteration_count {
        ...
        for joint_i in (1..n).rev() {
            ...
            let gp_rot = match grandparent_idx {
                Some(gp) if (gp.0 as usize) < current_global.len() => current_global[gp.0 as usize].rotation,
                _ => Quat::IDENTITY,
            };
            let delta_local = gp_rot.inverse() * delta_global * gp_rot;
...
fn apply_constraint(
    ...
    _original_global: &BoneTransform,
) {
    let rest = constraint
        .rest_angle
...
mod tests {
    ...
    fn linear_skeleton() -> (Skeleton, Vec<BoneIndex>) {
        ...
        let b1 = sk.add_bone(
            Some(root),
            "b1".into(),
            BoneTransform { translation: Vec3::Y, rotation: Quat::IDENTITY, scale: Vec3::ONE },
        );
        let b2 = sk.add_bone(
            Some(b1),
            "b2".into(),
    ...
    fn fabrik_out_of_reach() {
        ...
        assert!(solve_pose(&mut pose, &sk, &chain, &e, &cs));
        let g = pose.global_transforms(&sk);
        let d = g[bones[0].0 as usize].translation.distance(Vec3::new(0.0, 100.0, 0.0));
    ...
    fn zero_weight_pose_unchanged() {
        ...
        for i in 0..og.len() {
            assert!((og[i].translation - ng[i].translation).length() < 1e-6, "bone {} moved", i);
        }
    }
    ...
    fn arm_chain_reaches() {
        ...
        let upper = sk.add_bone(
            Some(shoulder), "upper".into(),
            BoneTransform { translation: Vec3::new(0.0, 0.3, 0.0), rotation: Quat::IDENTITY, scale: Vec3::ONE },
        );
        let forearm = sk.add_bone(
            Some(upper), "forearm".into(),
            BoneTransform { translation: Vec3::new(0.0, 0.3, 0.0), rotation: Quat::IDENTITY, scale: Vec3::ONE },
        );
        let hand = sk.add_bone(
            Some(forearm), "hand".into(),
            BoneTransform { translation: Vec3::new(0.0, 0.25, 0.0), rotation: Quat::IDENTITY, scale: Vec3::ONE },
        );
    ...
    fn constraint_limits_rotation() {
        ...
        cs.add(IkConstraint::new(bones[1]).with_twist(-5.0, 5.0).with_swing(-10.0, 10.0));
        assert!(solve_pose(&mut pose, &sk, &chain, &e, &cs));
    }
