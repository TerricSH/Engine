...
[assembly: AssemblyMetadata("EngineGameplay.ScriptApiVersion", "0.10.0")]
...
struct GeneratedScriptApiManifest {
    ...
    generated_sources: [&'static str; 2],
...
pub(crate) fn write_generated_script_api(
    ...
    let legacy_tactics_source = source_dir.join(engine_script_api::GENERATED_CSHARP_TACTICS_FILE);
    ...
    if legacy_tactics_source.is_file() {
        std::fs::remove_file(&legacy_tactics_source).map_err(|error| {
            format!(
                "could not remove legacy generated tactical Script API {}: {error}",
                legacy_tactics_source.display()
            )
        })?;
    }
