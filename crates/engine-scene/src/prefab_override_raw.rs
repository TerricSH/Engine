...
pub struct OverrideRecord {
    ...
    pub value: Value,
    /// The original value before the override was applied.
    /// Populated when the override is first created; used by
    /// [`revert_overrides`] to restore the prefab's base state.
...
pub fn revert_overrides(world: &mut World, overrides: &mut OverrideSet, instance_id: &str) {
    ...
    // For each reverted record we restore the original pre-override value
    // (snapshotted when the override was first applied), falling back to
    // the zero/default for the field type when no snapshot exists.
    for record in &reverted {
        ...
        // Use the stored original value if available; otherwise fall back
        // to the zero/default for the field type.
        let restore_val = match &record.original_value {
            Some(orig) => orig.clone(),
            None => default_value_for_field(&record.property_path),
        };
